...
import axios from "axios";
 ...
 if(localStorage.getItem("id") && localStorage.getItem("token")){
  ...
  return (<>
    {Data &&(<div>
        <h2 className='text-x1 font-semibold'>{Data.username}</h2>
     ...
     <div>
      {data.map((items, i)=>(
        <Link to={items.link} key={i} className='my-2 flex items-center hover:bg-gray-600 p-2 rounded transition-all duration-300 '>{items.icon} &nbsp;
        {items.title}</Link>
      ))}
     </div>
     <div>
